...
if (process.env.MONGODB_PASS) {
    uri = `mongodb://${process.env.MONGODB_USER}:${process.env.MONGODB_PASS}@${process.env.MONGODB_HOST}:${process.env.MONGODB_PORT}/jobs`;
} else {
    ...
    if (dateQuery !== 'any') {
        ...
        if (splitted.length === 1) {
            ...
            if (fromSplit.length !== 3) {
                ...
                fromDate = new Date(+fromSplit[2], +fromSplit[1] - 1, +fromSplit[0]).getTime();
...
export async function getStatistics() {
    ...
    const generalStats = await db.collection('statistics')
    ...
    let comboStats = await db.collection('statistics')
        .find({ type: 'combo' }, { projection: { '_id': 0, 'type': 0 } }).next();
