...
export default function Layout({ children }) {
  ...
  return (
    <div id={styles.layout}>
      <header
        id={styles.header}
        className={`${burgerToggle ? styles.burgerSticky : ""}`}
      >
        <div>
          {/* <svg
            xmlns="http://www.w3.org/2000/svg"
            viewBox={`${svgHeaderX} ${svgHeaderY} ${svgWidth} ${svgHeight}`}
          ...
          >
            <path
              fillOpacity="1"
              d="M0,288L80,250.7C160,213,320,139,480,128C640,117,800,171,960,192C1120,213,1280,203,1360,197.3L1440,192L1440,0L1360,0C1280,0,1120,0,960,0C800,0,640,0,480,0C320,0,160,0,80,0L0,0Z"
            ></path>
          </svg>
        </div>
        <div
          id={styles.headerNote}
          className={`${burgerToggle ? "hidden" : "visible"}`}
        >
          <span id={styles.logo}>&lt;/&gt;</span>
          <span>Explore statistics and search for software jobs.</span>
        </div>
        ...
        <div id={styles.burgerMenu}>
          <input
            id={`toggle-burger`}
            className={`${styles.menuTrigger} hidden`}
            type={`checkbox`}
            onClick={() => {
              setBurgerToggle((prev) => !prev);
            }}
          />
          <label className={styles.burgerWrapper} htmlFor={`toggle-burger`}>
            ...
            <animated.div id={styles.burgerCard} style={props}>
              <div id={styles.burgerContainer}>
                {Object.keys(navEntries).map((k) => (
                  <a
                    key={`${k}-burger`}
                    className={styles.burgerEntry}
                    href={navEntries[k].link}
                  >
                    <img alt={k} src={`/jobs/${k}.png`} />
                    {k}
                  </a>
                ))}
              </div>
              <div id={styles.burgerFooter}>
      ...
      <footer id={styles.footer}>
        {/* <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox={`${svgFooterX} ${svgFooterY} ${svgWidth} ${svgHeight}`}
          preserveAspectRatio={"none"}
        > */}
        <svg
          xmlns="http://www.w3.org/2000/svg"
          viewBox={`${svgFooterX} ${svgFooterY} 1400 600`}
          preserveAspectRatio={"none"}
        >
          <path
            fillOpacity="1"
        ...
        <div id={styles.footerNote}>
          <span>Web-design and maintenance by </span>
          <a href="https://niemal.dev/">niemal</a>
        </div>
